...
class WishlistController extends BaseController {
    ...
    public function handle(): void {
        ...
        try {
            ...
            Logger::error('Wishlists API error: ' . $e->getMessage());
            $this->serverError();
        }
    }
    ...
    private function index(): void {
        ...
        $stmt = $this->db->query("SELECT * FROM wishlists ORDER BY completed ASC, created_at DESC");
        $data = $stmt->fetchAll();
    ...
    private function create(): void {
        ...
        $stmt = $this->db->prepare(
            "INSERT INTO wishlists (title, description, date) VALUES (?, ?, ?)"
        );
    ...
    private function update(): void {
        ...
        $stmt = $this->db->prepare(
            "UPDATE wishlists SET title = ?, description = ?, date = ?, updated_at = NOW() WHERE id = ?"
        );
